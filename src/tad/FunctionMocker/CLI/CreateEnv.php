...
TEXT;
        ...
        $this->setName('generate:env')
             ...
             ->addArgument(
                 ...
                 null,
                 InputOption::VALUE_OPTIONAL,
                 'If set to `true` a `generation-config.json` file will be generated in the current '
                     . 'working directory.',
                 true
             )->addOption(
                 'with-dependencies',
                 null,
                 InputOption::VALUE_OPTIONAL,
                 'If this flag option is set than the command will try to find and pull in code '
                 . 'dependencies of the target code automatically searchin the specified sources.',
                 false
             )->addOption(
                 'author',
                 null,
    ...
    protected function initRunConfiguration()
    {
        ...
        $this->removeDocBlocks = empty($this->generationConfig['remove-docblocks']) ?
            false
            : (bool)$this->generationConfig['remove-docblocks'];
        ...
        $this->saveGenerationConfigFile = empty($this->generationConfig['save']) ?
            false
            : (bool)$this->generationConfig['save'];
        $this->functionsToFind = isset($this->generationConfig['functions']) ?
            $this->generationConfig['functions']
            : [];
        ...
        if ($this->classesToFindCount === static::NOTHING_TO_FIND
            && $this->functionsToFindCount === static::NOTHING_TO_FIND
        ) {
            $this->findAny = true;
        }
    ...
    protected function writeFunctionFile($data, $name, $namespace)
    {
        ...
        if (!\in_array($functionsFilePath, $this->openFunctionFiles, true)) {
            ...
            $this->writeFileHeaderToFile(
                $functionsFile,
                capitalPDangIt($this->generationConfig['name'])
                . ' environment '
    ...
    protected function writeEnvAutoloaderFile($name, $openingPhpTag)
    {
        ...
        ksort($classMap);
        $autoloadCode = $template->set(
            'header',
            $this->getFileHeader($name . ' environment autoloader.', 1)
        )
                                 ->set('id', $autoloaderName)
                                 ->set('name', capitalPDangIt($this->envName))->set('classMap', $classMap)
                                 ->render();
